...
const paidNoteUrl='https://note.com/dandy_heron6025/n/nf015146b9d11';
...
const websiteSchema={'@context':'https://schema.org','@type':'WebSite',name:siteName,url:`${domain}/`,description:'地方の現場職人が、下請けだけに頼らず自分で仕事を取れる状態を作るまでの実録。',inLanguage:'ja-JP',publisher:author};
const home=`${head({title:`${siteName}｜${subtitle}`,description:'地方の現場職人が、下請けだけに頼らず自分で仕事を取れる状態を作るまでの実体験、失敗、遠回り、途中経過を記録するブログ。',structuredData:websiteSchema})}<body>${header('./')}<main><section class="hero"><div class="wrap"><div class="eyebrow">地方の現場職人による実録</div><h1>${siteName}<span>下請け100%だった職人が、HPを公開して約3年。<br>今ではAI検索をきっかけに、問い合わせが来るようになりました。</span></h1><p>特別なことをしたわけではありません。投稿を忘れたり、サボったりもしました。それでも、できる時に少しずつ積み上げてきた実録です。</p></div></section>
...
<section class="wrap note-cta"><div><div class="eyebrow">PAID NOTE / 2,980円</div><h2>読んだ後、すぐ動ける形まで欲しい方へ</h2><p>施工事例、Googleマップ、口コミ、SNS、AI活用、ジモティまで。現場で実際に試してきた内容を、テンプレートとチェックリストにまとめています。</p><small>C-WEBでHP制作をご依頼いただいた方には、無料特典として提供予定です。</small></div><a href="${paidNoteUrl}" target="_blank" rel="noopener" data-note-event="paid_note_click">有料noteの内容を見る <span>↗</span></a></section>
...
function shortGuidePage(guide){
  ...
  return `${head({title:`${guide.title}｜${siteName}`,description:guide.description,path,type:'article',assetPrefix:'../../',structuredData:guideSchema(guide,path)})}<body data-guide-page="short">${header('../../')}<main><header class="guide-hero"><div class="wrap narrow"><div class="eyebrow">現場屋向け実務ガイド・無料版</div><h1>${escapeHtml(guide.h1)}</h1><p>${escapeHtml(guide.description)}</p></div></header><div class="wrap guide-layout"><article class="guide-body">${markdown(source)}<section class="guide-next"><span>詳細版</span><h2>もう少し詳しい理由や実体験まで読む</h2><p>なぜその運用が必要なのか、実際にやって分かったことまで詳しくまとめています。</p><a class="guide-button" href="full/" data-guide-event="guide_full_click">詳細版を読む →</a></section><section class="guide-next paid-note"><span>有料実行素材集・2,980円</span><h2>テンプレートまで、そのまま使いたい方へ</h2><p>施工事例文、Googleマップ投稿、口コミ依頼文、AIプロンプトなど、HP公開後の実行素材をまとめています。</p><a class="guide-button" href="${paidNoteUrl}" target="_blank" rel="noopener" data-note-event="paid_note_click">有料noteの内容を見る ↗</a><small>C-WEBでHP制作をご依頼いただいた方には、無料特典として提供予定です。</small></section><section class="guide-cweb-note"><h2>ホームページ制作そのものを相談したい方へ</h2><p>現場仕事を理解したホームページ制作については、C-WEBで案内しています。</p><a href="https://construction-web-terra.com/" data-guide-event="guide_cweb_click">C-WEBを見る →</a></section></article></div></main>${footer('../../')}`;
}
...
function fullGuidePage(guide){
  ...
  return `${head({title:`${guide.title}｜${siteName}`,description:guide.description,path,type:'article',assetPrefix:'../../../',structuredData:guideSchema(guide,path)})}<body data-guide-page="full">${header('../../../')}<main><header class="guide-hero full"><div class="wrap narrow"><div class="eyebrow">現場屋向け実務ガイド・詳細版</div><h1>${escapeHtml(guide.h1)}</h1><p>${escapeHtml(guide.description)}</p><a class="guide-back" href="../">← まず短縮版を読む</a></div></header><div class="wrap guide-layout"><article class="guide-body"><div class="guide-intro">${markdown(intro)}</div>${rendered}<section class="guide-next paid-note"><span>有料実行素材集・2,980円</span><h2>考え方だけでなく、実行素材まで欲しい方へ</h2><p>施工事例文、Googleマップ投稿、口コミ依頼文、AIプロンプトなど、HP公開後にそのまま使える形でまとめています。</p><a class="guide-button" href="${paidNoteUrl}" target="_blank" rel="noopener" data-note-event="paid_note_click">有料noteの内容を見る ↗</a><small>C-WEBでHP制作をご依頼いただいた方には、無料特典として提供予定です。</small></section><section class="guide-next cweb"><span>C-WEB</span><h2>HP制作そのものを相談したい方へ</h2><p>現場仕事を理解したホームページ制作について、必要なページ構成や進め方を案内しています。</p><a class="guide-button guide-cweb-link" href="https://construction-web-terra.com/" data-guide-event="guide_cweb_click">C-WEBを見る →</a></section></article></div></main>${footer('../../../')}`;
}
